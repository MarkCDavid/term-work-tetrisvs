...
static int _get_key_count(void)
{
    ...
    if (KEV.bKeyDown) {
        ...
        if (vk==VK_CAPITAL || vk==VK_NUMLOCK || vk==VK_SCROLL) {
            ...
            save_press = vk;
            switch (vk) {
            case VK_SHIFT:left_key = GetKeyState(VK_LSHIFT);
                break;
            case VK_CONTROL:left_key = GetKeyState(VK_LCONTROL);
                break;
            case VK_MENU:left_key = GetKeyState(VK_LMENU);
            }
        }
        else {
...
int PDC_get_key(void)
{
    ...
    if (key_count) {
        ...
        case KEY_EVENT:return _process_key_event();

        case MOUSE_EVENT:return _process_mouse_event();
